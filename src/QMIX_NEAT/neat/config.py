...
from src.QMIX_NEAT.neat.six_util import iterkeys
...
class Config(object):
    ...
    def __init__(self, genome_type, reproduction_type, species_set_type, stagnation_type, filename, env, args):
        ...
        num_agent = env.reset().shape[1]
        genome_dict["num_inputs"] = obs_dim + num_action + num_agent
        genome_dict["num_outputs"] = num_action
        genome_dict["num_hiddens"] = " ".join([*list(map(lambda x: str(x), args.before_rnn_layers)), *list(map(lambda x: str(x), args.after_rnn_layers[:-1]))])
