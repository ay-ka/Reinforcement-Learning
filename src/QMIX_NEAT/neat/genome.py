...
from src.QMIX_NEAT.neat.activations import ActivationFunctionSet
from src.QMIX_NEAT.neat.aggregations import AggregationFunctionSet
from src.QMIX_NEAT.neat.config import ConfigParameter, write_pretty_params
from src.QMIX_NEAT.neat.genes import DefaultConnectionGene, DefaultNodeGene
from src.QMIX_NEAT.neat.graphs import creates_cycle
from src.QMIX_NEAT.neat.six_util import iteritems, iterkeys
...
class DefaultGenomeConfig(object):
    ...
    def __init__(self, params):
        ...
        self._params = [ConfigParameter('num_inputs', int),
                        ...
                        ConfigParameter('num_hidden', list),
...
class DefaultGenome(object):
    ...
    def configure_new(self, config):
        ...
        if len(config.num_hidden) > 0:
            for hidden_layer_idx, hidden_layer in enumerate(config.num_hidden):
    ...
    def compute_full_connections(self, config, direct):
        ...
        """
        layer_index = 0
        hidden_layers_num_nodes = list(map(int, config.num_hidden))
        node_index = hidden_layers_num_nodes[layer_index]
        total_hiddens = OrderedDict()
        total_hiddens.update({"inputs":config.input_keys})
        hiddens = OrderedDict((layer_index, []) for layer_index, layer_num_nodes in enumerate(hidden_layers_num_nodes))
        for i in iterkeys(self.nodes):
            if i not in config.output_keys:
                hiddens[layer_index].append(i)
                node_index -= 1
            if node_index == 0:
                layer_index += 1
                try:
                    ...
                    break
        total_hiddens.update(hiddens)
        total_hiddens.update({"outputs" : config.output_keys})
        ...
        connections = []
        if config.num_hidden:
            layers_keys = list(total_hiddens.keys())
            len_layers = len(total_hiddens.keys())
            for i in range(len_layers - 1):
                for input_id in total_hiddens[layers_keys[i]]:
                    for middle_id in total_hiddens[layers_keys[i +1]]:
                        connections.append((input_id, middle_id))
        if direct or (not config.num_hidden):
