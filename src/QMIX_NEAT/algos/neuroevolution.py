...
import numpy as np
import math
import src.QMIX_NEAT.core.utils as utils
...
class SSNE:
	...
	def epoch(self, gen, pop, fitness_evals, migration):
		...
		if self.rl_policy != None:  # RL Transfer happened
			...
			self.rl_policy = None
			logger.log("EVOLUTION", f"-gen {gen}-num_elitists {num_elitists}-total_selection {self.selection_stats['total']}-as elite {self.selection_stats['elite']/self.selection_stats['total']}%-as offspring {self.selection_stats['selected']/self.selection_stats['total']}%-as discarded {self.selection_stats['discarded']/self.selection_stats['total']}% \n  ")
