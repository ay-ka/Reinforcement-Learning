...
class GymWrapper:
    ...
    """
    def __init__(self, env_name,  kwargs = {}, mode = "very_easy"):
        """
        A base template for all environment wrappers.
        """
        
        self.env_name = env_name
        self.num_agents = kwargs["agents"]
        
        
        if env_name == "RWARE":  
            if mode == "very_easy":
                self.env = gym.make("rware-tiny-" +  str(kwargs["agents"]) + "ag-v1")
            if mode == "easy":
                self.env = gym.make("rware-small-" +  str(kwargs["agents"]) + "ag-v1")
            if mode == "medium":
                self.env = gym.make("rware-medium-" +  str(kwargs["agents"]) + "ag-v1")
            if mode == "hard":
                self.env = gym.make("rware-large-" +  str(kwargs["agents"]) + "ag-v1")       
        elif env_name == "MPE":
            self.env = simple_spread_v2.env(N = kwargs["agents"], max_cycles=25, local_ratio = 0)
